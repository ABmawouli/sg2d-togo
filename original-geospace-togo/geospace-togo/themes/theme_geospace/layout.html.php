...
<head>
	<meta charset="UTF-8" />
	<title>Blog - GEOSPACE TOGO</title>
	...
	<style>
		...
		/* Cacher tous les liens et icônes RSS visibles sur le site */
		a[href*="rss"],
		a[href*="feed"],
		#icon-feed,
		svg#icon-feed,
...
<body class="centered-theme-layout <?php echo $occasioClass; ?> hfeed">
	...
	<!-- TITRE MIS AVEC CSS mon_style.css -->
	<section class="hero-blog">
		<div class="container">
			<h1>Actualités & Blog</h1>
			<p>Découvrez nos dernières informations, analyses et mises à jour.</p>
		</div>
	</section>
		...
		<main id="main" class="site-main" role="main">
			...
			<?php if (isset($is_category)): ?>
				<header class="entry-header">
					<h1 class="entry-title"><?php echo i18n('Category'); ?>: <?php echo $category->title; ?></h1>
					<div class="taxonomy-description"><?php echo $category->body; ?></div>
				</header>
			<?php endif; ?>
			<?php if (isset($is_tag)): ?>
				<header class="entry-header">
					<h1 class="entry-title"><?php echo i18n('Tags'); ?>: <?php echo $tag->title; ?></h1>
				</header>
			<?php endif; ?>
			<?php if (isset($is_archive)): ?>
				<header class="entry-header">
					<h1 class="entry-title"><?php echo i18n('Archives'); ?>: <?php echo $archive->title; ?></h1>
				</header>
			<?php endif; ?>
			<?php if (isset($is_search)): ?>
				<header class="entry-header">
					<h1 class="entry-title"><?php echo i18n('Search'); ?>: <?php echo $search->title; ?></h1>
				</header>
			<?php endif; ?>
			<?php if (isset($is_type)): ?>
				<header class="entry-header">
					<h1 class="entry-title">Type: <?php echo ucfirst($type->title); ?></h1>
				</header>
			<?php endif; ?>
		...
		<section id="secondary" class="sidebar widget-area" role="complementary">
			...
			<?php if (disqus()): ?>
				<section id="tzwb-recent-comments" class="widget tzwb-recent-comments">
					...
					<script src="//<?php echo config('disqus.shortname'); ?>.disqus.com/recent_comments_widget.js?num_items=5&amp;hide_avatars=0&amp;avatar_size=48&amp;excerpt_length=200&amp;hide_mods=0" type="text/javascript"></script>
					<style>
						li.dsq-widget-item {
							padding-top: 15px;
						}
			...
			<!-- Widget 1 : WhatsApp -->
			<div id="widget-whatsapp" class="widget widget_links">
				<h3 class="widget-title">Contactez-nous</h3>
				<a href="https://wa.me/22893215014" target="_blank" class="social-link whatsapp-link">
	...
	<div id="footer-placeholder">
		<?php include 'mes-includes/footer.php'; ?>
	</div>
	<script type="text/javascript" src="<?php echo theme_path(); ?>js/custom.js" id="custom-js"></script>
	<script type="text/javascript" src="<?php echo theme_path(); ?>js/maison/favicon-script.js"></script>
